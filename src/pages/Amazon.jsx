...
export default function Amazon() {
  ...
  async function fetchData() {
    try {
      ...
      Object.keys(benchmarksData).forEach(key => {
        benchData[key] = benchmarksData[key].amazon || '';
      });
  ...
  useEffect(() => {
    // Don't set up observer if all products are displayed
    ...
    // Wait a bit for DOM to update after products render
      ...
      observer = new IntersectionObserver(
        (entries) => {
          ...
          if (entry.isIntersecting && !isLoadingMore) {
            // Double check we still have more to load
        ...
        {
          threshold: 0.1,
          rootMargin: '500px' // Start loading 500px before reaching the bottom
        }
      );
  ...
  return (
    <div className="min-h-screen" key="amazon-v1">
      <Header />
      <main className="container mx-auto px-4 py-6 max-w-7xl">
        {/* Compact Header */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <img src="/amazonlogo.png" alt="Amazon" className="w-10 h-10" />
            <div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {displayedProducts.map((product, idx) => {
                const benchmark = benchmarks[product.name];
            ...
            {/* Sentinel element for infinite scroll - always render when more products available */}
