...
export default function Amazon() {
  ...
  useEffect(() => {
    sessionStorage.removeItem('amazon-cache');
    setDisplayLimit(50);
  ...
  async function fetchData() {
    try {
      ...
      setBenchmarks(benchmarksData.amazon || {});

      const modeForToday = calendarData?.[today]?.amazon || 'BAU';
  ...
  const { filteredProducts, displayedProducts } = useMemo(() => {
    ...
    if (filterBy === 'instock') {
      ...
      filtered = filtered.filter(p => {
        ...
        const reference = pricingMode === 'EVENT' ? bench.event : bench.bau;
  ...
  return (
    <div className="container mx-auto px-10 pt-[56px] pb-8 flex-1 flex flex-col" key="amazon-v1">
      {/* Compact Header */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 mb-10 overflow-hidden">
        <div className="flex items-center gap-5">
          <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center p-3 shadow-xl shadow-black/20">
            <img src="/amazonlogo.png" alt="Amazon" className="w-full h-full object-contain" />
          </div>
          <div>
            <h1 className="text-3xl font-medium text-white tracking-tight">Amazon Tracking</h1>
