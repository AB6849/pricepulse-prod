...
export default function Swiggy() {
    ...
    async function fetchData() {
        try {
            ...
            // Determine today's pricing mode (Note: pricing_calendar table uses "swiggy" as key for Instamart)
            const modeForToday =
                calendarData?.[today]?.swiggy || 'BAU';

            setPricingMode(modeForToday);

            // Keep benchmarks mapping to instamart as current table uses it
    ...
    return (
        <div className="container mx-auto px-10 pt-[56px] pb-8 flex-1 flex flex-col" key="instamart-v1">
            {/* Compact Header */}
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 mb-10 overflow-hidden">
                <div className="flex items-center gap-5">
                    <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center p-3 shadow-xl shadow-black/20">
                        <img src="/instamart_logo.webp" alt="Instamart" className="w-full h-full object-contain" />
                    </div>
                    <div>
                        <h1 className="text-3xl font-medium text-white tracking-tight">Instamart Tracking</h1>
                            ...
                            {displayedProducts.map((product, idx) => {
                                ...
                                return (
                                    <a
                                        ...
                                        onClick={() => sessionStorage.setItem('swiggy-scroll', window.scrollY)}
                                        className="group"
                                    >
                                        <div className="glass-card product-card flex flex-col h-full hover:shadow-lg hover:shadow-indigo-500/5 relative">
                                            {/* Top Accent */}
                                            {/* isSelected is not defined in the provided context, removing it */}
                                            {/* {isSelected && <div className="absolute top-0 left-0 right-0 h-[2px] bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.5)]" />} */}
