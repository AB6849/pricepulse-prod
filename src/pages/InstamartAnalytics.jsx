...
export default function InstamartAnalytics() {
    ...
    const loadAnalytics = async () => {
        ...
        try {
            const { summary, cities, risk, trend, facilities, nuclear } = await getCompleteAnalytics('swiggy', brandSlug);
    ...
    const handleUpload = async (file, type) => {
        ...
        if (type === 'sales') {
            return await uploadSalesData('swiggy', brandSlug, rows, user.id);
        } else {
            return await uploadInventoryData('swiggy', brandSlug, rows, user.id);
        }
    };

    const handleCheckOverlap = async (file, type) => {
        const rows = await parseCSV(file);
        return await checkExistingDates('swiggy', brandSlug, rows, type);
    };
        ...
        <div className="space-y-6">
            ...
            {activeTab === 'upload' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <CSVUploader onUpload={handleUpload} type="sales" onCheckOverlap={handleCheckOverlap} platform="swiggy" />
                    <CSVUploader onUpload={handleUpload} type="inventory" onCheckOverlap={handleCheckOverlap} platform="swiggy" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                            <div>
                                ...
                                <code className="text-zinc-400 block bg-black/30 p-3 rounded-lg overflow-x-auto">
                                    BRAND, ORDERED_DATE, CITY, L1_CATEGORY, PRODUCT_NAME, VARIANT, ITEM_CODE, UNITS_SOLD, GMV
                                </code>
                            </div>
                            <div>
                                <p className="text-emerald-400 font-bold mb-2 uppercase tracking-wider">Inventory CSV</p>
                                <code className="text-zinc-400 block bg-black/30 p-3 rounded-lg overflow-x-auto">
                                    FacilityName, City, SkuCode, SkuDescription, L2, DaysOnHand, PotentialGmvLoss, OpenPos, OpenPoQuantity, WarehouseQtyAvailable
                                </code>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="glass-card p-6">
                            <div className="flex flex-col xl:flex-row xl:items-center justify-between mb-6 gap-6">
                                <div className="flex items-center gap-4">
                                    <h3 className="text-white text-sm font-bold whitespace-nowrap">Inventory by Feeder House</h3>
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-white/[0.02] text-zinc-500 text-[10px] uppercase font-black tracking-widest border-b border-white/5">
                                            ...
                                            <th className="px-6 py-4 text-center cursor-pointer hover:text-white transition-colors" onClick={() => setNuclearSort({ column: 'open_po_qty', order: nuclearSort.column === 'open_po_qty' && nuclearSort.order === 'asc' ? 'desc' : 'asc' })}>
                                                <div className="flex items-center justify-center gap-2">
                                                    Supply Pipeline (PO)
                                                    {nuclearSort.column === 'open_po_qty' && (nuclearSort.order === 'asc' ? ' ↑' : ' ↓')}
                                                </div>
                                            </th>
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {sortedNuclearItems.map((item) => (
                                            <tr key={item.item_id} className="group hover:bg-white/[0.02] transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex flex-col items-center">
                                                        <p className={`text-sm font-bold ${(item.open_po_qty || 0) > 0 ? 'text-emerald-400' : 'text-zinc-500'}`}>
                                                            {(item.open_po_qty || 0).toLocaleString('en-IN')}
                                                        </p>
                                                        {item.open_pos && item.open_pos.length > 0 && (
                                                            <span className="text-[9px] bg-white/5 px-1.5 py-0.5 rounded mt-1 text-zinc-400 border border-white/5">
                                                                {item.open_pos.length} POs
                                                            </span>
                                                        )}
                                                    </div>
                                                </td>
