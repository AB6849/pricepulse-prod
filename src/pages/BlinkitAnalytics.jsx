...
export default function BlinkitAnalytics() {
    ...
    const loadAnalytics = async () => {
        ...
        try {
            const { summary, cities, risk, trend, facilities, nuclear } = await getCompleteAnalytics('blinkit', brandSlug);
    ...
    const handleUpload = async (file, type) => {
        ...
        if (type === 'sales') {
            return await uploadSalesData('blinkit', brandSlug, rows, user.id);
        } else {
            return await uploadInventoryData('blinkit', brandSlug, rows, user.id);
        }
    };

    const handleCheckOverlap = async (file, type) => {
        const rows = await parseCSV(file);
        return await checkExistingDates('blinkit', brandSlug, rows, type);
    };
        ...
        <div className="space-y-6">
            ...
            {activeTab === 'upload' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <CSVUploader onUpload={handleUpload} type="sales" onCheckOverlap={handleCheckOverlap} platform="blinkit" />
                    <CSVUploader onUpload={handleUpload} type="inventory" onCheckOverlap={handleCheckOverlap} platform="blinkit" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                            <div>
                                ...
                                <code className="text-zinc-400 block bg-black/30 p-3 rounded-lg overflow-x-auto">
                                    item_id, item_name, manufacturer_id, manufacture city_id, city_name, category, date, qty_sold, mrp
                                </code>
                            </div>
                            <div>
                                <p className="text-emerald-400 font-bold mb-2 uppercase tracking-wider">Inventory CSV</p>
                                <code className="text-zinc-400 block bg-black/30 p-3 rounded-lg overflow-x-auto">
                                    created_at, backend_facility_name, backend_fac, item_id, item_name, backend_inv, frontend_inv
                                </code>
                            </div>
                        </div>
                    </div>
